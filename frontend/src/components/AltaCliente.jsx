...
export default function AltaCliente({ apiBase, setError, showSuccess }) {
  ...
  const { items: sortedData, requestSort, getSortIcon } = useSortableData(filteredData, { key: 'ID_CLIENTE', direction: 'desc' });
    ...
    <div className="space-y-6 animate-fade-in flex flex-col h-full">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Gestión de base de datos de clientes</p>
        </div>
        ...
        {loadingList ? (
          ...
          <table className="min-w-full text-left text-sm whitespace-nowrap">
            <thead className="uppercase tracking-wider border-b-2 border-gray-200 bg-gray-50">
              <tr>
                <th className="px-6 py-4 cursor-pointer hover:bg-gray-200 transition-colors" onClick={() => requestSort('ID_CLIENTE')}>ID{getSortIcon('ID_CLIENTE')}</th>
                ...
                <th className="px-6 py-4 cursor-pointer hover:bg-gray-200 transition-colors" onClick={() => requestSort('DESC_ESTADO')}>Estado{getSortIcon('DESC_ESTADO')}</th>
        ...
        <div className="fixed inset-0 bg-gray-900/60 backdrop-filter backdrop-blur-sm flex items-center justify-center z-[100] transition-all duration-300">
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-scale-in">
            ...
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3 rounded-b-2xl">
              ...
              <button type="submit" form="form-cliente" disabled={saving} className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-medium shadow-md hover:from-blue-700 hover:to-indigo-700 cursor-pointer transition-all disabled:opacity-50">
